...
def _answer_sort_key(ans: ParsedAnswer) -> tuple[int, int, str]:
    m = re.match(r"P\.(\d+)", ans.respondent_id)
    if m:
        return (0, int(m.group(1)), ans.respondent_id)
    m2 = re.match(r"回答者(\d+)", ans.respondent_id)
    if m2:
        return (1, int(m2.group(1)), ans.respondent_id)
    return (2, 0, ans.respondent_id)


def format_consolidated_text(questions: list[ParsedQuestion]) -> str:
    """構造化した質問・回答を集約テキスト形式に戻す."""
    if not questions:
        return ""
    lines: list[str] = []
    for q in sorted(questions, key=lambda x: x.q_num):
        lines.append(f"Q{q.q_num}: {q.q_text}")
        if q.answers:
            for ans in sorted(q.answers, key=_answer_sort_key):
                lines.append(f"・[{ans.respondent_id}] {ans.text}")
        else:
            lines.append("・（無回答）")
        lines.append("")
    return "\n".join(lines).rstrip()
...
def parse_page_snippet_edits(snippet: str, page_num: int) -> dict[int, str]:
    """ページ抜粋テキストから設問番号→回答文の dict を抽出."""
    ...
    for line in (snippet or "").splitlines():
        ...
        if not s:
            continue
        m = _Q_HEADER_RE.match(s)
        if m:
            if current_q is not None and buf:
                updates[current_q] = " ".join(buf).strip()
            current_q = int(m.group(1))
            buf = []
            continue
        am = _ANSWER_LINE_RE.match(s)
        if am and current_q is not None:
            updates[current_q] = am.group(2).strip()
            buf = []
            continue
...
def merge_page_snippet_into_consolidated(
    full_text: str,
    page_num: int,
    new_snippet: str,
) -> str:
    """インライン校正: 1 ページ分の抜粋修正を集約テキストへ反映."""
    questions = parse_consolidated_text(full_text)
    if not questions:
        return new_snippet.strip() or full_text
    rid = f"P.{page_num}"
    updates = parse_page_snippet_edits(new_snippet, page_num)
    for q in questions:
        q.answers = [a for a in q.answers if a.respondent_id != rid]
        ans = updates.get(q.q_num, "").strip()
        if ans and ans != "（無回答）":
            q.answers.append(ParsedAnswer(respondent_id=rid, text=ans))
    return format_consolidated_text(questions)
